...
def case_files(subject_dir: Path, require_label: bool = True) -> dict:
...
def read_partitioning(data_root: str | Path, partition_csv: str | Path) -> list[tuple[str, list[dict]]]:
    ...
    for partition_id, frame in table.groupby("Partition_ID", sort=True):
        records = [case_files(root / str(subject_id)) for subject_id in frame["Subject_ID"]]
...
def make_loaders(
    ...
    train_recs, val_recs = split_records(records, seed=seed)
    if cache_rate:
        train_ds = CacheDataset(train_recs, transform=train_transforms(), cache_rate=cache_rate)
        val_ds = CacheDataset(val_recs, transform=val_transforms(), cache_rate=cache_rate)
    else:
        train_ds = Dataset(train_recs, transform=train_transforms())
        val_ds = Dataset(val_recs, transform=val_transforms())
    return (
...
class FetsDatasetLoader(BaseDatasetLoader):
    """Loader class adhering to maam's BaseDatasetLoader interface."""

    def __init__(self, root_dir: str, partition_csv: str = ""):
        super().__init__(root_dir)
        self.partition_csv = partition_csv

    def load(self):
        """Returns full dataset partition records."""
        return read_partitioning(self.root_dir, self.partition_csv)

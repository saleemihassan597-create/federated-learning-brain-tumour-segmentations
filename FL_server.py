...
def run_global_test(final_state_dict: dict, config) -> dict[str, float]:
    ...
    strategy_name = str(config.get("strategy", config.get("algorithm", "fedavg"))).lower()
    output_dir = Path(config.get("output-dir", "artifacts"))
    output_dir.mkdir(parents=True, exist_ok=True)
    output_file = output_dir / f"{strategy_name}_fets2022_global_test.csv"
    ...
    print(
        ...
        f"Pred/Target WT Voxels: {results['pred_et_voxels']:.0f} / {results['target_wt_voxels']:.0f}"
    )
    return results
...
def main(grid: Grid, context: Context) -> None:
    ...
    result = strategy.start(
        ...
        train_config=ConfigRecord({"lr": float(config["learning-rate"]), "proximal-mu": float(config.get("proximal-mu", 0.0))}),
    ...
    # Run final global test evaluation
